...
def generate_recommendations():
    ...
    for _, row in freq_candidates.iterrows():
        if row['utilization'] > 1.0 or row['gap_status'] == 'capacity_shortfall':
            pri = "Critical" if row['overload_day_share'] >= rec_rules.get("critical_occupancy_threshold", 0.95) else "High"
            add_rec(
                subject_id=row['route_id'],
                category="FREQUENCY",
                action="Increase frequency during identified peak periods.",
                evidence=f"Route {row['route_id']} ({row['time_period']}): avg utilization {row['utilization']:.1%}, overloaded on {row['overload_day_share']:.1%} of observed days. Required capacity: {row['required_capacity']:.0f}, current extra trips suggested: {row['extra_trips_per_hour'] if pd.notna(row['extra_trips_per_hour']) else 1}.",
                priority=pri,
                impact=f"(Estimate) Adding {row['extra_trips_per_hour'] if pd.notna(row['extra_trips_per_hour']) else 1} trips/hr will reduce occupancy below critical thresholds and resolve {row['gap_status']}."
            )

    # CAPACITY recommendations
    cap_candidates = demand_supply_gap[demand_supply_gap['suggestion'].astype(str).str.contains('larger_vehicle', case=False, na=False)]
    for _, row in cap_candidates.iterrows():
        ...
        add_rec(
            ...
            evidence=f"Route {row['route_id']} ({row['time_period']}): p90 load is {row['p90_load']:.1f}, suggested vehicle type: {row['suggested_vehicle_type']}.",
            priority=pri,
            impact=f"(Estimate) Providing {row['suggested_capacity']} capacity per trip will eliminate {row.get('denied_per_trip', 0)} denied boardings per trip."
        )
    ...
    for _, row in schedule_adherence.iterrows():
        ...
        if late_share > rec_rules.get("critical_delay_trip_pct", 0.30):
            add_rec(
                subject_id=row['route_id'],
                category="SCHEDULE",
                action="Shift departure time or adjust dwell time.",
                evidence=f"Route {row['route_id']}: {late_share:.1%} of trips are late, well above the 30% critical threshold.",
                priority="Critical",
                impact="(Estimate) Adjusting schedule will improve on-time performance by up to 25%."
            )
        elif early_share > 0.20 or late_share > 0.15:
            add_rec(
                subject_id=row['route_id'],
                category="SCHEDULE",
                action="Shift departure time or adjust dwell time.",
                evidence=f"Route {row['route_id']}: {early_share:.1%} early and {late_share:.1%} late.",
                priority="Low",
                impact="(Estimate) Minor schedule adjustments will correct adherence deviations."
            )

    # STOP recommendations
    for _, row in stop_performance[stop_performance['is_bottleneck'] == True].iterrows():
        late_rate = row['late_record_rate']
        pri = "Critical" if late_rate > rec_rules.get("critical_bottleneck_delay_pct", 0.20) else "High"
        add_rec(
            subject_id=row['stop_id'],
            category="STOP",
            action="Investigate stop, extend dwell time, or review stop sequence.",
            evidence=f"Stop {row['stop_id']} ({row['stop_name']}): late record rate {late_rate:.1%}, average delay {row['avg_record_delay_min']:.1f} mins.",
            priority=pri,
    ...
    recurring_anomalies = anomaly_counts[anomaly_counts['count'] > 1]
    for _, row in recurring_anomalies.iterrows():
        add_rec(
            subject_id=row['entity_id'],
            category="ANOMALY",
            action="Investigate root cause of recurring anomaly.",
            evidence=f"Entity {row['entity_id']} experienced {row['anomaly_type']} {row['count']} times.",
            priority="Medium",
    ...
    for _, row in route_reliability.iterrows():
        if row['on_time_rate'] < rec_rules.get("high_reliability_threshold", 0.70):
            add_rec(
                subject_id=row['route_id'],
                category="RELIABILITY",
                action="Review schedule or add buffer time.",
                evidence=f"Route {row['route_id']}: on-time rate is only {row['on_time_rate']:.1%}, travel time CV is {row.get('travel_time_cv', 0):.2f}.",
                priority="High",
    ...
    for _, row in underutilized_services.iterrows():
        if row.get('p90_occupancy', 1) < rec_rules.get("medium_low_occupancy_threshold", 0.30):
            add_rec(
                subject_id=row['route_id'],
                category="FREQUENCY",
                action="Reduce trips or assign lower capacity vehicle.",
                evidence=f"Route {row['route_id']} ({row['time_period']}): p90 occupancy is {row.get('p90_occupancy', 0):.1%}, utilization status: {row.get('utilization_status', 'Low')}.",
                priority="Medium",
